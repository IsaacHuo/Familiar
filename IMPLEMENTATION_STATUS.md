...
- SwiftData V1/V2 保持冻结；当前 V3 通过轻量迁移新增 Project Resource、版本和不可变 Run ContextSnapshot 引用，V2 store 可原位升级，打开失败时仍提供用户确认的数据恢复流程。
...
- 2026-08-14 使用 iOS 26.5 arm64 Simulator 运行完整测试：51 项通过、0 失败；参数化 Benchmark 的 8 个场景全部通过。
...
- Artifact、Binding、Memory、Resource 工具和 Resource 后续版本更新尚未实现；WP4 第一版只支持项目文档首次导入、预览、删除和自动上下文注入。
