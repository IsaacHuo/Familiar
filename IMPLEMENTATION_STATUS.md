...
- SwiftData 会话、消息、附件、模型切换和运行记录；V2 store 打开失败时提供用户确认的数据恢复流程。
...
- iOS 单元测试、UI 冷启动冒烟测试、AnyDoc Rust fixture 测试和网站构建流程。

## 当前验证

- `FamiliarWidgets` Debug arm64 iOS Simulator 构建通过。
- `Familiar` Debug arm64 iOS Simulator 构建及扩展嵌入校验通过。
- iOS 单元测试共 23 项，覆盖 Provider/路由/收件箱/附件边界、Agent Runtime、EventKit 写入策略、Run/Step 与 store 恢复等关键逻辑。
...
- 实现完整 Run 重放/恢复、后台恢复事件和 `BGContinuedProcessingTask`；当前通知不保证 App 挂起后任务继续运行。
