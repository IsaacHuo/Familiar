...
private enum Vision {
    ...
    fileprivate class VisionModelCoreML {
        ...
        func load() throws -> MLModel {
            try lock.withLock {
                ...
                let compiledURL = modelDirectory.appendingPathComponent("fastvithd.mlmodelc", isDirectory: true)
                let packageURL = modelDirectory.appendingPathComponent("fastvithd.mlpackage", isDirectory: true)
                let modelURL: URL
        ...
        public func encode(_ image: MLXArray) -> MLXArray {
            ...
            return data.withUnsafeMutableBytes { (ptr: UnsafeMutableRawBufferPointer) in
                ...
                // inference
                let provider = try! MLDictionaryFeatureProvider(dictionary: ["images": MLFeatureValue(multiArray: array)])
                let prediction = try! model().prediction(from: provider)
                let output = prediction.featureValue(for: "image_features")!.multiArrayValue!
                precondition(output.shape == [1, 256, 3072])
                precondition(output.dataType == .float32)
                return output.withUnsafeBytes { ptr in
    ...
    fileprivate class VisionModel: Module {
        ...
        public init(modelDirectory: URL) {
            model = VisionModelCoreML(modelDirectory: modelDirectory)
            super.init()
        }
...
public class FastVLM: Module, VLMModel, KVCacheDimensionProvider {
    ...
    public init(_ config: FastVLMConfiguration, modelDirectory: URL) {
        self.config = config
        self._visionModel.wrappedValue = Vision.VisionModel(modelDirectory: modelDirectory)
