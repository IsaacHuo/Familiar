...
|---|---|
| UI | SwiftUI |
| 数据模型 | SwiftData（单一 `FamiliarReleaseSchema`，36 实体，无 migration plan） |
...
- `FamiliarArtifactTool.swift` — 保留 `artifact_write/edit`；`artifact_publish` 只从当前 Project Workspace Outputs 导入经过扩展名、文件签名、可解析正文、必需内容和 hash 校验的真实文件。
