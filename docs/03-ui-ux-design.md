...
- 左对齐。
- 无气泡正文排版。
- 流式与终态正文都使用 `FamiliarMarkdownWebView`，首帧或渲染失败时使用原生回退文本。
- 一个 Assistant Turn 是按 Runtime `sequence` 排列的内容流：模型正文块、工具执行块、确认介入和后续模型正文保持实际发生顺序。
- 每轮模型输出在进入工具前形成独立 `ResponseBlock`；实时与历史使用同一顺序投影。
- 每个工具调用只有一个稳定 UI 身份，从运行中原位更新为确认、结果、失败、取消或已撤销，不在正文末尾重复生成结果卡。
...
运行期间显示当前最有信息量的一行状态，例如“模型正在思考”“正在查阅资料”“正在识别图片”。状态文案不使用卡片垫底，不暴露内部协议或模型推理内容。只读工具结果在对应正文位置显示一行摘要，完成后默认折叠，点击后从当前行向下展开。回答末尾的 Activity 只显示状态、工具数与耗时，不承载审计明细。
...
- 按字段排序的完整参数。
- 当前阶段与终态。
- 首次授权时的“仅这次 / 本次会话 / 始终允许”，默认“仅这次”。高风险、删除和 Shell 类动作只能选择“仅这次”。
- 明确的风险等级、后果与撤销能力。
- 取消、确认、失败重试或撤销等当前阶段允许的动作。

动作卡位于时间线内，保持与模型回答相同的上下文位置。有效授权范围内不重复询问，但执行结果仍保留授权来源。失败保留原因与重试入口；撤销后保留“已撤销”终态，不从时间线删除。多个动作按调用顺序纵向排列，不使用横向 pager。
...
| 色彩 | 用途 |
|---|---|
| Apple system blue | 主要动作、选中状态、发送 |
...
展开折叠保持顶部锚点不动，只动画容器高度和透明度，不使用从上方或下方移入的 transition，也不对展开内容做逐行位移。Reduce Motion 开启时立即切换。
