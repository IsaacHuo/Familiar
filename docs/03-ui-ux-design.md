...
图片发送边界随模型能力逐步开放。设计约束：只向支持图片的模型发送图片，且图片字节只发往用户所选 Provider。

- 模型不支持图片时：允许选择、拍摄、预览和移除图片，但发送统一拦截。
- 拦截时：不创建用户消息、不发起 Provider 请求、不清空文字草稿、不清空图片草稿，并显示当前模型不支持图片的说明。
- 模型支持图片时：图片进入请求；适配器负责图片编码（当前 Anthropic / Gemini 图片编码在 `state/CURRENT.md` 的 In Progress 中）。
